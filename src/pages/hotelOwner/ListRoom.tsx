        ...
        <table className='w-full'>
          <thead className='bg-gray-50'>
            <tr>
              ...
              <th className='py-3 px-4 text-gray-800 font-medium max-sm:hidden'>Sacility</th>
          ...
          <tbody className='text-sm'>
            {
              rooms.map((item, index) => (
                <tr key={index}>
                  <td className='py-3 px4 text-gray-700 border-t border-gray-300'>
                    {item.roomType}
                  </td>
                  ...
                  <td className='py-3 px-4 border-t border-gray-300 text-sm text-red-500 text-center'>
                    <label htmlFor="" className='relative inline-flex items-center cursor-pointer text-gray-900 gap-3'>
                      ...
                      <div className='w-12 h-7 bg-slate-300 rounded-full peer peer-checked:bg-blue-600 transition-colors duration-300'>
                        <span className={`dot absolute ${!item.isAvailable ? 'right-1' : 'left-1'} top-1 w-5 h-5 bg-white rounded-full transition-transform duration-200 ease-in-out peer-checked:translate-x-5`}></span>
                      </div>
                    </label>
                  </td>
                </tr>
